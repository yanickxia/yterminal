...
pub async fn agent_service_status() -> Result<AgentServiceStatus, String> {
    let running = tokio::net::UnixStream::connect(paths::socket_path())
        .await
        .is_ok();
...
pub async fn install_agent_service() -> Result<AgentServiceStatus, String> {
    let running = tokio::net::UnixStream::connect(paths::socket_path())
        .await
        .is_ok();
    tokio::task::spawn_blocking(move || -> Result<AgentServiceStatus, String> {
        install_binary()?;
        install_manager_definition(running)?;
        Ok(status_impl(running))
    })
    .await
...
pub async fn start_agent_service() -> Result<AgentServiceStatus, String> {
    ...
    while tokio::net::UnixStream::connect(paths::socket_path())
        .await
        .is_ok()
    {
    ...
    loop {
        if tokio::net::UnixStream::connect(paths::socket_path())
            ...
            .is_ok()
        {
            return tokio::task::spawn_blocking(|| status_impl(true))
                .await
                .map_err(|error| format!("agent status task: {error}"));
        }
        if tokio::time::Instant::now() >= deadline {
