...
- **AppImage IME env.** A webkit2gtk AppImage runs without the desktop's `GTK_IM_MODULE` exported, so GTK never connects to the IME and no candidate window appears. `ensure_ime_env()` in `src-tauri/src/main.rs` runs `#[cfg(target_os = "linux")]` at the very top of `main()` (before `tauri::Builder`, i.e. before GTK init): only when `GTK_IM_MODULE` is unset, it detects the IME (from `XMODIFIERS`' `@im=` hint, else a `ps -eo comm` scan for `fcitx5`/`fcitx`/`ibus-daemon`) and sets `GTK_IM_MODULE`/`QT_IM_MODULE`/`XMODIFIERS`. No IME detected → nothing set (pure-ASCII users unaffected).
