...
- **Transcript + wire (transient store)** — `src/stores/ai-store.ts` (not persisted; scoped to the running session like the terminal itself) holds `turns` (roles `user`/`assistant`/`tool`), `sending`, `open`, `agentMode`, `pendingApproval`, plus `send(prompt, context?, paneId?)`, `stop()`, `setAgentMode`, and `resolveApproval(approved)`. `send` optimistically appends a user turn + a pending assistant turn, builds wire messages `[system prompt] (+ optional context as a fenced system message) + prior non-pending/non-error/non-tool turns`, then branches: **chat mode** streams via `aiChatStream`, appending deltas onto the pending turn and tracking `currentStreamId` so `stop()` can `aiCancel` it; **agent mode** runs `runAgentLoop` — a bounded (`MAX_AGENT_STEPS=12`) tool loop that calls `aiChatTools` with the `run_command` JSON-Schema tool, and for each requested command awaits `requestApproval` (a module-side promise the UI resolves), runs it via `runCommandInPane` when approved, renders a `tool` turn (command + exit code + output), and feeds a `tool`-role result message keyed by `tool_call_id` back into the loop until the model stops calling tools. `src/lib/ai.ts` wraps all IPC: `aiChat`, `aiChatStream`/`aiCancel` (Channel-based), `aiChatTools`, plus the `AiTool`/`AiToolCall`/`AiAssistantMessage` types.
- **UI** — `src/components/AiSidebar.tsx` renders as a sibling in `.app` when `ai-store.open`; assistant replies render through the shared `renderMarkdown` (marked → DOMPurify) used by the file viewer, so model output is sanitized before injection. An "Agent" checkbox toggles agent mode; while streaming (chat mode) the Send button becomes a **Stop** control; `tool` turns show the command + exit-code badge + scrollable output, and a `pendingApproval` renders an inline Approve/Deny prompt. `App.tsx` refits the active tab's panes when the sidebar toggles (it changes terminal width). Styles are the `.ai-sidebar*` / `.ai-msg*` / `.ai-tool-*` / `.ai-approval*` / `.ai-provider-*` rules in `styles.css`.
