...
- **Sound** — `lib/alert-sound.ts` synthesizes a two-blip chime via WebAudio (no bundled asset), self-throttled to one play per 1.5s so a chatty TUI redraw doesn't machine-gun the speaker. `playAlertSound(force)` bypasses the throttle for the Settings preview button. Gated by `settings.alertSoundEnabled` (default on).
