...
- **HTTP proxy (Rust)** — the WebView can't call LLM endpoints directly (CORS + it would expose the key in page context), so the proxy commands in `src-tauri/src/main.rs` are thin OpenAI-compatible passthroughs. Three variants: `ai_chat` (non-streaming, returns `choices[0].message.content`); `ai_chat_stream` (P2) POSTs `stream:true` and pushes SSE deltas back over a Tauri `ipc::Channel<AiStreamEvent>` (`Delta{text}` / `Done` / `Error{message}` — provider errors are delivered as terminal `Error` events, never as `Err`, so the channel always terminates cleanly), with an `ai_chat_cancel(streamId)` that flips a shared `AtomicBool` tracked in a `static AI_CANCELS` registry + RAII `CancelGuard`; `ai_chat_tools` (P3) POSTs non-streaming with a `tools` array + `tool_choice:"auto"` (only when tools are non-empty) and returns the raw `choices[0].message` verbatim so the caller can branch on `content` vs `tool_calls`. Uses `reqwest` 0.12 with `rustls-tls` + the `stream` feature and `futures-util` (`bytes_stream()` + `StreamExt`) to parse SSE `data:` lines.
